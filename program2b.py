...
class Program2(UAVController):
	...
	def decision_making(self):
		...
		if self.state=="takeoff":
			...
			if current_time>self.control_start_time+20:
				...
				self.roll_zero,self.pitch_zero=0,0#roll,pitch
			...
			if current_time>self.switch_att_time+3:
				x,y,z,_,_,_,_=self.parse_local_position()
			...
			self.acrobat_cmd["r"]+=(angv_r+action[0])/50.0
			self.acrobat_cmd["p"]+=(angv_p+action[1])/50.0
			self.acrobat_cmd["z"]+=(vz+action[2]*0.8)/50.0
	...
	def user_control_reset(self):
		...
		self.pidz=PID(kp=7,ki=0.5/50.0,kd=0.01*50,out_min=-3,out_max=3,out_init=0)
		# self.pidzv=PID(kp=0.3,ki=0.06/50.0,kd=0.003*50,out_min=0,out_max=1,out_init=0.55)
		self.pidzv=PID(kp=0.3,ki=0.1/50.0,kd=0.003*50,out_min=0,out_max=1,out_init=0.55)
